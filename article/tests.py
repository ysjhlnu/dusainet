...
class ArticlesPostModelTests(TestCase):
    ...
    def test_increase_views(self):
        ...
        self.assertEqual(article.created, article.updated)

    def test_updated_field_change(self):
        """更新文章后updated字段是否更新"""
        article = ArticlesPost(title='test_updated_field_change', body='test_updated_field_change', author=create_author())
        article.save()
        time.sleep(0.1)
        article.save()
